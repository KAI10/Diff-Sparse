...
def get_params():
    ...
    parser.add_argument("--data", type=str, default='TIDEWATCH_64_10')
    parser.add_argument("--mask_ratio", type=float, default=0.0) # mask of history data
    parser.add_argument("--noise_mask", type=float, default=0.0)
...
def default_config(data='AIR_BJ'):
    ...
    if config.data.name.startswith('TIDEWATCH'):
        print('dataset',config.data.name)
        config.data.num_features = 1
        config.data.points_per_hour = 1
        config.data.val_start_idx = 1921
        config.data.test_start_idx = 1945
        params = (config.data.name).split('_')
        patch_size = int(params[1])
        patch_no = int(params[2])
        #print(patch_size*patch_size*patch_no)
        config.data.num_vertices = patch_size*patch_size*patch_no
        print('parsed info num vertices',config.data.num_vertices)
    ...
    config.epoch = 40  # Number of max training epoch
...
def evals(model, data_loader, epoch, metric, config, clean_data, mode='Test'):
    print('\nentering evals with mode',mode)
    setup_seed(2022)
    
    mask_path = config.data.path + config.data.name + '/mask.npy'
    ...
    for i, batch in enumerate(data_loader):
        ...
        #n_samples = 1 if mode == 'Val' else 2#config.n_samples
...
#     print('\ny_true final shape in eval',y_true.shape)
#     print('\ny_pred final shape in eval',y_pred.shape)
    
#     np.save(config.data.path + config.data.name + '/y_true.npy',y_true)
#     np.save(config.data.path + config.data.name + '/y_pred.npy',y_pred)
    
    time_cost = np.sum(time_lst)
    metric.update_metrics(y_true, y_pred)
    metric.update_metrics_v2(y_true, y_pred, y_mask)
    print(f"mode {mode} |[nrmse {metric.metrics_v2['nrmse']:<7.8f} nacrps {metric.metrics_v2['nacrps']:<7.8f}]")
    ...
    print('reached end of call to evals with mode',mode)
    return metric
...
def main(params: dict):
    ...
    NOISE_PERCENTAGE = params['noise_mask']
    ...
    train_dataset = TrafficDataset(clean_data, (0 + config.model.T_p, config.data.val_start_idx - config.model.T_p + 1), config, mode='train',noise_percentage=NOISE_PERCENTAGE)
    train_loader = torch.utils.data.DataLoader(train_dataset, config.batch_size, shuffle=True, pin_memory=True)

    val_dataset = TrafficDataset(clean_data, (config.data.val_start_idx + config.model.T_p, config.data.test_start_idx - config.model.T_p + 1), config, mode='valid',noise_percentage=NOISE_PERCENTAGE)
    ...
    test_dataset = TrafficDataset(clean_data, (config.data.test_start_idx + config.model.T_p, -1), config, mode='test',noise_percentage=NOISE_PERCENTAGE)
    ...
    #print('total epoch is',config.epoch)
    #print('params[is_train]',params['is_train'])
