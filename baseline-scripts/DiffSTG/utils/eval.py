...
def masked_mse_np_v2(y_true, y_pred, y_mask):
    mse = (y_true - y_pred) ** 2
    return np.sum(y_mask * mse)/np.sum(y_mask)
...
class Metric(object):
    ...
    def update_metrics_v2(self, y_true, y_pred, y_mask):
        """
        both y_true and y_pred should be numpy
        :param y_true: (B, T_p, V, D)
        :param y_pred: (B, n_samples, T_p, V, D) or (B, T_p, V, D)
        ...
        assert  isinstance(y_true, np.ndarray) and isinstance(y_pred, np.ndarray), \
            f"y_true and y_pred should be np.ndarray, now its type is y_true:{type(y_true)}, y_pred:{type(y_pred)}"
        self.metrics_v2 = {'nrmse': 0.0, 'nacrps': 0.0, 'time': 0.0}

        if y_pred.shape == y_true.shape: #  y_true: (B, T_p, V, D) and y_pred: (B, T_p, V, D)
            y_pred = np.expand_dims(y_pred, axis = 1) # (B, 1, T_p, V, D)

        # probabilistic metric
        self.metrics_v2['nacrps'] = calc_NACRPS(y_true, y_pred, y_mask)
        
        # deterministic metric
        y_pred = np.mean(y_pred, axis=1) # # (B, T_p, V, D)

        self.metrics_v2['nrmse']= self.get_nrmse(y_true, y_pred,y_mask)
        # self.metrics['time'] = time_to_str((timer() - self.time_start))
        self.metrics_v2['time'] = timer() - self.time_start
    ...
    @staticmethod
    def get_nrmse(y_true, y_pred,y_mask):
        #y_true shape (b,t,v,d)
        #mae = masked_mae_np(y_true, y_pred, 0)
        target_dim = y_true.shape
        y_mask = repeat(y_mask, 'v -> b t v d',b=target_dim[0],t=target_dim[1],d=target_dim[3])        
        mse = masked_mse_np_v2(y_true, y_pred, y_mask)
        #mape = masked_mape_np(y_true, y_pred, 0)
        min_y_true = y_true.flatten()[(y_mask.astype(bool)).flatten()].min()
        max_y_true = y_true.flatten()[(y_mask.astype(bool)).flatten()].max()
        rmse = (mse ** 0.5)
        nrmse = (rmse)/(max_y_true-min_y_true)
        return nrmse
...
def calc_NACRPS(target, forecast, mask):
    """
    target: (B, T, V), torch.Tensor
    forecast: (B, n_sample, T, V), torch.Tensor
    eval_points: (B, T, V): which values should be evaluated,
    ...
    target = rearrange(target, 'b t v d -> t (b v d)')
    forecast = rearrange(forecast, 'b s t v d -> t s (b v d)')
    mask = repeat(mask,'v -> b v',b=target_dim[0])
    mask = rearrange(mask, 'b v -> (b v)')
    ## convert mask to booltype
    mask = mask.astype(bool)
    
    #print('target shape before masking',target.shape)
    masked_target = target[:,mask]
    #print('target shape after masking',masked_target.shape)
    
    #print('forecast shape before masking',forecast.shape)
    masked_forecast = forecast[:,:,mask]
    #print('forecast shape after masking',masked_forecast.shape)
    
    masked_forecast = rearrange(masked_forecast, 't s v -> (t v) s')
    masked_target = rearrange(masked_target, 't v -> (t v)')
    masked_crps = ps.crps_ensemble(masked_target, masked_forecast)
    return masked_crps.sum()/np.sum(np.absolute(masked_target))
