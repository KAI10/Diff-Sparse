...
class TrafficDataset(Dataset):
    def __init__(self, clean_data, data_range, config, mode='train',noise_percentage=0.0): ## add a mode here for 'train/test'
        ...
        self.test_noise_mask = None
        if self.noise_percentage>0.0:
            self.test_noise_mask = (torch.load(f'{config.data.path}{config.data.name}/test_masks_for_sparsity-{self.noise_percentage}.pt')).numpy()
        self.land_mask = np.load(f'{config.data.path}{config.data.name}/mask.npy')
        self.land_mask = (self.land_mask).astype('int8')
    ...
    def __getitem__(self, index): ## add masking script here, if mode train, mask from script, if mode test, mask from data

        recent_idx = self.idx_lst[index]
        
        start, end = recent_idx[1][0], recent_idx[1][1]
        label = self.label[start:end] ## horizon
        ...
        if self.mode=='train' or self.mode=='val':
            rand_mask = np.random.uniform(size=(node_feature.shape[1],node_feature.shape[2]))
            data_mask = np.ones((node_feature.shape[1],node_feature.shape[2]),dtype='int8') # v,1
            data_mask[rand_mask<self.noise_percentage] = 0
            data_mask = (data_mask.flatten())*self.land_mask
            data_mask = repeat(data_mask,'v-> c v f',c=node_feature.shape[0],f=node_feature.shape[2])
            noise_mask = np.random.randn(*data_mask.shape).astype('float32')
            
            node_feature = node_feature * data_mask + ~data_mask * noise_mask
        
        if self.mode=='test' and self.test_noise_mask is not None:
            if self.printed<self.max_print:
                print('Loading precomputed test noise mask for sparsity')
                
            tot_mask = self.test_noise_mask.shape[0]
            current_mask = self.test_noise_mask[index%tot_mask].flatten()
            no_patches = node_feature.shape[1]//(current_mask.shape[0])
            current_mask  = repeat(current_mask,'v -> (p v)',p=no_patches)
            current_mask = current_mask*self.land_mask
            current_mask = repeat(current_mask, 'v -> c v f',c = node_feature.shape[0], f = node_feature.shape[2])
            noise_mask = np.random.randn(*current_mask.shape).astype('float32')
            node_feature = node_feature * current_mask + ~current_mask * noise_mask
