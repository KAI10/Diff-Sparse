...
def get_train_dataset(config, patch_origins):
    train_dataset = TidewatchTimeseriesDataset(
        ...
        add_elevation_channel=True,
    )

    multi_patch_train_dataset = MultiPatchDataset(
        train_dataset,
        patch_size=config.patch_size,
        num_unique_patches=config.num_unique_patches,
...
def get_test_dataset(config, patch_origins, multi_patch_train_dataset):
...
if __name__ == "__main__":
    ...
    multi_patch_test_dataset = get_test_dataset(config, patch_origins, multi_patch_train_dataset)
    ...
    # for _, (context, covariate, horizon, land_mask, context_inundation_median, context_inundation_iqr) in enumerate(train_dataloader):
    #     print(context.shape, covariate.shape, horizon.shape, land_mask.shape, context_inundation_median.shape, context_inundation_iqr.shape)
    #     if torch.any(torch.isnan(context)):
    #         print("NaN values exist in the context")
    #         exit(1)

    #     if torch.any(torch.isnan(horizon)):
    #         print("NaN values exist in the horizon")
    #         exit(1)
    ...
    with trainer.init_module():
        ...
        patch_embedding_model = PatchEmbeddingNet(config=config)
        ...
        # model = LightningDiffusionModel.load_from_checkpoint(
        #     "output/64/p10_closs_0/seed_0/epoch=27-val_masked_nrmse=0.064226.ckpt",
    ...
    ###############  Testing  ###################
    # config.horizon_length = 12
    # json.dump(dataclass_to_dict(config), open(config.store_path + 'test_config.json', 'w'), default=str)

    # best_model_path = "/project/biocomplexity/ki5hd/DDPM-Coastal-Flooding/models/x_parameterization/small_model/patch_size/64/p10_closs_0/seed_0/epoch=24-train_loss=0.030533.ckpt"
