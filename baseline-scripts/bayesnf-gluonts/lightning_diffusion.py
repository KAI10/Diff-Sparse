...
class LightningDiffusionModel(LightningModule):
    def __init__(self, diffusion_model: SpatioTemporalDDPM, config: TrainingConfig):
        ...
        # self.val_crps = 0
        # self.val_gt_abs_sum = 0
        # self.val_squared_error = 0
        # self.val_gt_num = 0
        # self.val_gt_min, self.val_gt_max = 1e9, -1e9
    ...
    def training_step(self, batch, batch_idx):
        ...
        x0_theta = self.diffusion_model.backward(noisy_imgs, context, covariate, t.reshape(n, ))

        # predicted_initial_image = self.diffusion_model.denoise(noisy_imgs, eta_theta, t)
        elevation = context[:, 0, 1:, :, :]  # Grabbing elevation from context
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = StepLR(optimizer, step_size=self.config.lr_update_step_size,
    ...
    def process_validation_test_batch(self, batch, batch_idx, mode):
        ...
        batch_masked_crps, batch_masked_gt_abs_sum, \
             batch_masked_se, batch_masked_gt_min, batch_masked_gt_max, batch_masked_gt_num = self.get_batch_masked_metric(
            denormalized_x, denormalized_horizon, horizon_mask.bool()
        )

        return batch_masked_crps, batch_masked_gt_abs_sum, batch_masked_se, batch_masked_gt_min, batch_masked_gt_max, batch_masked_gt_num
    ...
    # def get_batch_crps(self, predictions, ground_truth):
    #     # print(predictions.shape, ground_truth.shape)
    #     x = rearrange(predictions, 'b s d c h w -> (b d c h w) s')
    #     gt = rearrange(ground_truth, 'b d c h w -> (b d c h w)')
    #     # print(x.shape, gt.shape)
    #     crps = ps.crps_ensemble(gt.cpu(), x.cpu())
    #     return crps.sum(), gt.abs().sum()

    # def get_batch_masked_crps(self, prediction, ground_truth, mask):
    #     x = rearrange(prediction, 'b s d c h w -> d s (b c h w)')
    #     gt = rearrange(ground_truth, 'b d c h w -> d (b c h w)')

    #     mask = rearrange(mask, 'b h w -> (b h w)')
    #     masked_x = x[:, :, mask]
    #     masked_gt = gt[:, mask]

    #     masked_x = rearrange(masked_x, 'd s v -> (d v) s')
    #     masked_gt = rearrange(masked_gt, 'd v -> (d v)', )
    #     masked_crps = ps.crps_ensemble(masked_gt.cpu(), masked_x.cpu())

    #     return masked_crps.sum(), masked_gt.abs().sum()

    # def get_batch_squared_error(self, prediction, ground_truth):
    #     mean_x = reduce(prediction, 'b s d c h w -> b d c h w', 'mean')
    #     error = torch.square(ground_truth - mean_x).sum()
    #     return error, ground_truth.min(), ground_truth.max(), ground_truth.numel()

    # def get_batch_masked_squared_error(self, prediction, ground_truth, mask):
    #     x = rearrange(prediction, 'b s d c h w -> d s (b c h w)')
    #     gt = rearrange(ground_truth, 'b d c h w -> d (b c h w)')
    ...
    #     mean_masked_x = reduce(masked_x, 'd s v -> d v', 'mean')
    #     error = torch.square(masked_gt - mean_masked_x).sum()
    #     return error, masked_gt.min(), masked_gt.max(), masked_gt.numel()
