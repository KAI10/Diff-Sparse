...
class SampleForecastGenerator(ForecastGenerator):
    ...
    def __call__(
        ...
        **kwargs
        ...
        ## code added for sanity check, prepare for masking
        if verbose:
            print('Inside sample forecast generator invoked from predict method')
        batch_printed = 0
        max_batch_print = 0
        for batch in inference_data_loader:
            inputs = select(input_names, batch, ignore_missing=True)
            ...
            ## code added for sanity check, prepare for masking
            if verbose and batch_printed<=max_batch_print:
                batch_printed = batch_printed+1
                print(f'processed {batch_printed} batches')
                print(f'inputs have following keys')
                for key in inputs:
                    print(key)
                    print(f'shape of inputs[{key}] is {inputs[key].shape}')
                print('\n')
                print(f'output type is {type(outputs)}')
                print(f'shape of outputs is {outputs.shape}')
                #print(f'input shape is {outputs.shape}')
