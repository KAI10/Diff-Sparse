...
class Trainer:
    ...
    def __init__(
        ...
        ##custom variables added for understanding training nature
        self.printed = 0
        self.max_print = 3
        ...
        with tempfile.TemporaryDirectory(
            ...
            def loop(  # todo call run epoch
                ...
                for batch_no, batch in enumerate(it, start=1):
                    ...
                    #code snippet added for verbosity
#                     if self.printed<self.max_print:
#                         print('inside Trainer train loop of gluonts')
#                         print(f'currently working with batch no {batch_no} of epoch {epoch_no}')
#                         batch_keys = [k for k in batch]
#                         print(f'batch dictionary has the following keys: {batch_keys}')
#                         print('shape of the batch keys are as follows')
#                         for k in batch_keys:
#                             print(f'shape of batch[{k}] is {batch[k].shape}')
#                         self.printed += 1
                    
#                     # code for masking for LSTNET
#                     if self.model_name=='LSTNET':
#                         before_mask_batch = batch['past_target'].copy()
#                         _b,_f,_t = batch['past_target'].shape[0],batch['past_target'].shape[1],batch['past_target'].shape[2]
#                         data_mask = np.ones((_b,_f))
#                         random_chance_matrix = np.random.uniform(size=(_b,_f))
#                         condition_to_mask_land_cell = (random_chance_matrix < self.mask_percentage)
#                         data_mask[condition_to_mask_land_cell] = 0
#                         data_mask = repeat(data_mask,'b f -> b f t',t=_t)
#                         data_mask = mx.nd.array(data_mask)
#                         batch['past_target'] = batch['past_target']*data_mask
#                         if self.noise_mask:
#                             noise_for_masking = mx.nd.array(np.random.normal(size=data_mask.shape))
#                             batch['past_target'] += (1-data_mask)*noise_for_masking 
                        
#                         # for sanity check only
#                         diff = batch['past_target']-before_mask_batch
#                         if self.printed<self.max_print:
#                             print(f'after masking {self.mask_percentage}% difference is {diff.sum()}')
                    any_batches = True
