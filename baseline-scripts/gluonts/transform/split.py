...
class InstanceSplitter(FlatMapTransformation):
    ...
    def __init__(
        ...
        self.mode = mode
        self.mask_data_patch_size = mask_data_patch_size
        
        print(f'past length in instance splitter in mode {self.mode} is {self.past_length}')
        
        
        print(f'Instancesplitter initialized for noise % = {self.noise_percentage}, mode = {self.mode}, patch size = {self.mask_data_patch_size}')
        
        if self.mode=='test' and self.noise_percentage>0.0:
            mask_dir = '/project/biocomplexity/Diff_Spatio_Temporal_Grid/data/Diff-Flood-Sparse-Test-Masks'
            mask_data_path = f'{mask_dir}/test_masks_for_sparsity-{self.mask_data_patch_size}-{self.noise_percentage}.pt'
            self.test_noise_mask_data = (torch.load(mask_data_path)).numpy()
    ...
    def _split_array(
        ...
        #     print('past piece type',type(past_piece))
        #     print('future piece type',type(future_piece))
        #     print('past piece shape',past_piece.shape)
        #     print('future piece shape',future_piece.shape)
        #     self.printed += 1
    ...
    def _split_instance(self, entry: DataEntry, idx: int) -> DataEntry:
        ...
        entry = entry.copy()
        #if self.mode=='test':
        #    print('split instance idx is',idx)
        for ts_field in slice_cols:
            
            past_piece, future_piece = self._split_array(entry[ts_field], idx)
            if self.printed<self.max_print:
                print('self mode is',self.mode)
                print('ts_field',ts_field)
                print('past piece shape',past_piece.shape)
                print('future piece shape',future_piece.shape)
                print('first few values of past',past_piece[0:4,0:4])
                print('first few values of future',future_piece[0:4,0:4])
                self.printed += 1
                
            if ts_field == 'target' and self.noise_percentage > 0.0:
                # past_piece v x c
                if self.mode=='training' or self.mode=='validation':
                    if self.printed<self.max_print:
                        print('applying noise to past piece with noise percentage',self.noise_percentage)
                    num_vars = past_piece.shape[0]
                    rand_mask = np.random.uniform(size=(num_vars))
                    data_mask = np.ones((num_vars),dtype='int8') # v,1
                    data_mask[rand_mask<self.noise_percentage] = 0
                    data_mask = repeat(data_mask,'v -> v c',c=past_piece.shape[1])
                    noise_mask = np.random.randn(*data_mask.shape).astype('float32')
                    
                    past_piece = past_piece * data_mask + ~data_mask * noise_mask
                
                if self.mode=='test' and self.noise_percentage>0.0:
                    #print('using precomputed mask data to perform forecast')
                    tot_mask = self.test_noise_mask_data.shape[0]
                    current_noise_mask = self.test_noise_mask_data[(idx-1956)%tot_mask].flatten()
                    current_noise_mask = repeat(current_noise_mask, 'v -> v c',c = past_piece.shape[1])
                    noise_mask = np.random.randn(*current_noise_mask.shape).astype('float32')
                    past_piece = past_piece * current_noise_mask + ~current_noise_mask * noise_mask
