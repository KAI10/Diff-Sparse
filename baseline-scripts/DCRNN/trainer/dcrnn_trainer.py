...
class DCRNNTrainer(BaseTrainer):
    ...
    def __init__(self, model, loss, metrics, optimizer, config, data_loader,
                 valid_data_loader=None, lr_scheduler=None, len_epoch=None, val_len_epoch=None, scaler=None):
    ...
    def _valid_epoch(self, epoch):
        ...
        mask_data = np.load(f"/project/biocomplexity/Diff_Spatio_Temporal_Grid/DCRNN/pytorch-DCRNN/{self.config['dataloader']['args']['data_dir']}/mask_val.npy")
        ...
        all_labels = np.vstack(all_labels)#rearrange(np.array(all_labels),'n b h v f -> (n b) h v f')
        all_outputs = np.vstack(all_outputs)#rearrange(np.array(all_outputs),'n b h v f -> (n b) h v f')
        print('all label shape',all_labels.shape)
        print('all output shape',all_outputs.shape)
        
        all_labels = self.scaler.inverse_transform(all_labels)
        all_outputs = self.scaler.inverse_transform(all_outputs)
        
        with open(f"/project/biocomplexity/Diff_Spatio_Temporal_Grid/DCRNN/pytorch-DCRNN/{self.config['dataloader']['args']['data_dir']}/labels.npy", 'wb') as f:
            np.save(f, all_labels)
        with open(f"/project/biocomplexity/Diff_Spatio_Temporal_Grid/DCRNN/pytorch-DCRNN/{self.config['dataloader']['args']['data_dir']}/outputs.npy", 'wb') as f:
            np.save(f, all_outputs)
        
        normalize_val = np.nanmax(all_labels)-np.nanmin(all_labels)
        
        all_labels = all_labels[0:13,:,:,:]*mask_data
        all_outputs = all_outputs[0:13,:,:,:]*mask_data
        
        their_nrmse = self._eval_metrics(all_outputs, all_labels).tolist()[-1]/normalize_val
        
        our_nrmse = (np.sqrt(np.sum(np.square(all_labels-all_outputs))/np.sum(mask_data)))/normalize_val
        
        print('final validation nrmse loss',their_nrmse,our_nrmse)
